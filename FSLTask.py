...
def get_datasetFeaturesFiles(dsname, method, split): 
    return "./checkpoints/%s/%s/last/%s_features.plk"%(dsname,method, split)

def loadDataSet(dsname, method, split):
    if dsname not in ['miniImagenet','CUB','cifar', 'cross', 'tieredImagenet']:
    ...
    # home = expanduser("~")
    dataset = _load_pickle(get_datasetFeaturesFiles(dsname,method, split))
...
#        return
#
#    rsFile = os.path.join(_cacheDir, "RandStates_{}_s{}_q{}_w{}_m{}_{}_{}".format(
#        dsName, cfg['shot'], cfg['queries'], cfg['ways'],cfg['method'],cfg['split']))
...
def GenerateRunSet(start=None, end=None, cfg=None):
    ...
    if cfg is None:
        cfg = {"shot": 1, "ways": 5, "queries": 15, "method": 'WideResNet28_10_S2M2_R'}
...
if __name__ == "__main__":
    ...
    loadDataSet('miniimagenet','WideResNet28_10_S2M2_R')
